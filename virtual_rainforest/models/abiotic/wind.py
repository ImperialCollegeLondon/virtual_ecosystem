...
import numpy as np
import xarray as xr
from numpy.typing import Tuple
...
def calculate_wind_profile(
    ...
    * leaf area index
    ...
    # Calculate profiles and return as Tuple
...
def calculate_wind_above_canopy(
    ...
    We follow the implementation by :cite:t:`Campbell2012`.
    ...
    wind_profile_above = DataArray(
        np.zeros(wind_heights.size, friction_velocity.size), dims=["height", "cell_id"]
    )
    for z in wind_heights:
        wind_profile_above[z,] = (friction_velocity / 0.4) * xr.ufuncs.log(
            wind_heights[
                z,
            ]
            - zero_plane_displacement / roughness_length_momentum
        ) + diabatic_correction_momentum_above
    return wind_profile_above
...
def calculate_wind_below_canopy(
    ...
    """
    topofcanopy_wind_speed = wind_profile_above[
        -1,
    ]
    wind_profile_canopy = DataArray(
        np.zeros(canopy_node_heights.size, topofcanopy_wind_speed.size),
        dims=["canopy_layer", "cell_id"],
    )
    for z in canopy_node_heights:
        wind_profile_canopy[z,] = topofcanopy_wind_speed * xr.ufuncs.exp(
            wind_attenuation_coefficient((canopy_node_heights / canopy_height) - 1)
        )
    return wind_profile_canopy
...
def calculate_zero_plane_displacement(
    ...
    return (
        ...
        - (
            1
            - xr.ufuncs.exp(
                -xr.ufuncs.sqrt(zero_plane_scaling_parameter * plant_area_index)
            )
        )
        / xr.ufuncs.sqrt(zero_plane_scaling_parameter * plant_area_index)
    ) * canopy_height
...
def calculate_roughness_length_momentum(
    ...
    leaf_area_index: DataArray,
    zero_plane_displacement: DataArray,
    substrate_surface_drag_coefficient=WindConstants.substrate_surface_drag_coefficient,
    roughness_element_drag_coefficient=WindConstants.roughness_element_drag_coefficient,
    roughness_sublayer_depth_parameter=WindConstants.roughness_sublayer_depth_parameter,
    max_ratio_wind_to_friction_velocity=WindConstants.max_ratio_wind_to_friction_velocity
    )-> DataArray:
    ...
    ratio_wind_to_friction_velocity = (
        xr.ufunc.sqrt(substrate_surface_drag_coefficient + (roughness_element_drag_coefficient * plant_area_index) / 2)
    )

    # if ratio of wind velocity to friction velocity larger than maximum, set to maximum
    where (ratio_wind_to_friction_velocity > max_ratio_wind_to_friction_velocity):
        ratio_wind_to_friction_velocity = max_ratio_wind_to_friction_velocity

    roughness_length = (
        (canopy_height - zero_plane_displacement) 
        * xr.ufunc.exp(-0.4 * (1 / ratio_wind_to_friction_velocity) 
...
def calculate_diabatic_correction_momentum_above(
    ...
    Diabatic correction factors are used in adjustment of wind profiles and calculation
    of turbulent conductivity above canopy.
    
    Args:
        temperature: temperature # TODO find out at which height
        atmospheric_pressure: atmospheric pressure, [kPa]
        sensible_heat_flux: sensible heat flux, [J m-2] # TODO find out at which height
...
#' @return `psi_h` diabatic correction factor for heat transfer
    ...
    specific_heat_air = calc_specific_heat_air(temperature=temperature)#  cp
...
def calculate_mixing_length(
    ...
    return (
        ...
        / xr.ufunc.log((canopy_height - zero_plane_displacement)
...
def calculate_wind_attenuation_coefficient(
    ...
    diabatic_correction_factor_below: float = WindConstants.diabatic_correction_factor_below,
    )-> DataArray:
    ...
    return (
        ...
        / (2 * mixing_length * relative_turbulence_intensity))^0.5
