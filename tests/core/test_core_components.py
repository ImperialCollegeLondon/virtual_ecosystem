    ...
    argvalues=[
        ...
        pytest.param(
            ...
            (
                (
                    ERROR,
                    "Maximum depth of soil layers is less than the biological topsoil "
                    "depth",
                ),
            ),
...
def test_LayerStructure_init(
    caplog, config_string, biotic_topsoil_depth, raises, expected_values, expected_log
):
    ...
    with raises:
        layer_structure = LayerStructure(
            cfg.core.layers, n_cells=9, biotic_topsoil_depth=biotic_topsoil_depth
        )
...
def test_LayerStructure_set_filled_canopy():
    ...
    * Checks that the aggregate role index has been updated with the new canopy state.
    ...
    layer_structure = LayerStructure(
        core_cfg.layers, n_cells=9, biotic_topsoil_depth=0.25
    )
