...
ds_1cid = xarray.Dataset(
    data_vars={
        "temp": xarray.DataArray(temp.flatten(), coords=[cell_id], dims=["cell_id"]),
        "prec": xarray.DataArray(prec.flatten(), coords=[cell_id], dims=["cell_id"]),
    }
)
...
ds_1cid_badid = xarray.Dataset(
    data_vars={
        "temp": xarray.DataArray(temp.flatten(), coords=[cell_id_no], dims=["cell_id"]),
        "prec": xarray.DataArray(prec.flatten(), coords=[cell_id_no], dims=["cell_id"]),
    }
)
...
# ----------------------------------------------------------------------
# Generate NetCDF with column data with a row_idx dimension integer, _not_ including x
# and y data (A data frame, basically, but with an assumed order).
# ----------------------------------------------------------------------
