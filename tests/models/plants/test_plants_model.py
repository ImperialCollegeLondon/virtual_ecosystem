...
def test_PlantsModel_calculate_mycorrhizal_uptakes(fxt_plants_model):
    ...
    # Check that all expected variables are generated and have the correct value
    assert np.allclose(fxt_plants_model.data["plant_n_uptake_arbuscular"], 0.5)
    assert np.allclose(fxt_plants_model.data["plant_n_uptake_ecto"], 0.5)
    assert np.allclose(fxt_plants_model.data["plant_p_uptake_arbuscular"], 0.5)
    assert np.allclose(fxt_plants_model.data["plant_p_uptake_ecto"], 0.5)
