"""Configuration for the palms model."""
...
class PalmsConstants(Configuration):
    """Constants for the :mod:`~virtual_ecosystem.models.palms` model.
...
class PalmsExportConfig(Configuration):
    """Configuration class for plant export options.

    The palms model only writes a relatively small amount of data to the central data
    ...
    * Cohort data: details about the stems in each cohort, including the stem allometry
      ...
      This data is exported to the file ``palms_cohort_data.csv``.
    ...
    * Community canopy data: community wide data on the canopy structure, such as the
      ...
      exported to the file ``palms_community_canopy_data.csv``.
...
class PalmsConfiguration(ModelConfigurationRoot):
    """Root configuration class for the palms model."""
    ...
    community_data_export: PalmsExportConfig = PalmsExportConfig()
    """Configuration of plant community data export"""
    constants: PalmsConstants = PalmsConstants()
    """Constants for the palms model"""
