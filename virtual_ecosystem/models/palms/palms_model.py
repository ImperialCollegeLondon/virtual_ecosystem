"""The :mod:`~virtual_ecosystem.models.palms.palms_model` module creates
:class:`~virtual_ecosystem.models.palms.palms_model.PalmsModel` class as a child of
the :class:`~virtual_ecosystem.core.base_model.BaseModel` class.

This is a standalone growth-form model for palms, analogous to
:class:`~virtual_ecosystem.models.palms.palms_model.PalmsModel`, but using
height-driven palm growth equations from :mod:`~virtual_ecosystem.models.palms.palms`
in place of the DBH-driven T Model equations from :mod:`pyrealm.demography.tmodel`.
"""  # noqa: D205
...
from virtual_ecosystem.models.palms.communities import PalmCommunities, PalmCommunity
from virtual_ecosystem.models.palms.functional_types import get_flora_from_config
from virtual_ecosystem.models.palms.model_config import (
    PalmsConfiguration,
    PalmsConstants,
)
from virtual_ecosystem.models.palms.palms import (
    GrowthIncrements,
    StemAllocation,
    StemAllometry,
)
...
class PalmsModel(
    ...
    The palms model is initialised using data from three sources:
    ...
    2. A data frame defining the initial cohort inventories for each grid cell. Each row
        ...
        * ``palm_cohorts_pft``: The PFT of the cohort, matching an entry in the
          ``flora``
        * ``palm_cohorts_cell_id``: The grid cell id containing the cohort
        * ``palm_cohorts_n``: The number of individuals in the cohort
        * ``palm_cohorts_stem_height``: The stem height of the individuals in
          metres.

    These data are used to setup the plant communities within each grid cell, using the
    :class:`~virtual_ecosystem.models.palms.communities.PalmCommunities` class to
    ...
    * the whole canopy leaf mass within the layers (``layer_leaf_mass``)
    ...
    used to predict the change in stem height expected during the update interval.
    ...
    Args:
        ...
        extra_pft_traits: Additional traits for each plant functional type, keyed by
            PFT name.
        model_constants: Set of constants for the palms model.
    ...
    def __init__(
        ...
        model_constants: PalmsConstants = PalmsConstants(),
        ...
        self.flora: Flora
        """A flora containing the plant functional types used in the palms model."""
        self.initial_cohort_data: pandas.DataFrame
        """A dataframe providing the initial cohort data."""
        self.cohort_id_generator: Iterator
        """Set of constants for the palms model"""
        self.model_constants: PalmsConstants
        """Set of constants for the palms model"""
        self.communities: PalmCommunities
        """An instance of PalmCommunities providing dictionary access keyed by cell id
    ...
    def _setup(
        ...
        """Setup implementation for the Palms Model.
        ...
        if not set(self.data["plant_pft_propagules"]["pft"].data) == set(
            ...
            raise InitialisationError(
                ...
                "the PFT names configured in the PalmsModel flora"
            )
        ...
        self.canopies = calculate_canopies(
            communities=cast(PlantCommunities, self.communities),
        ...
        self.subcanopy = Subcanopy(
            ...
            model_constants=cast(PlantsConstants, self.model_constants),
    ...
    def from_config(
        ...
        """Factory function to initialise a palms model from configuration.

        This function returns a PalmsModel instance based on the provided configuration
        ...
        model_configuration: PalmsConfiguration = configuration.get_subconfiguration(
            "palms", PalmsConfiguration
        )
        ...
        try:
            ...
            LOGGER.critical(f"Error creating palms model from configuration: {excep!s}")
            raise excep

        LOGGER.info("Palms model instance generated from configuration.")
        return inst

    def spinup(self) -> None:
        """Placeholder function to spin up the palms model."""
    ...
    def cleanup(self) -> None:
        """Placeholder function for palms model cleanup."""
    ...
    def apply_herbivory(self) -> None:
        ...
        for cell_id in self.grid.cell_id:
            community: PalmCommunity = self.communities[cell_id]
    ...
    def estimate_gpp(self, time_index: int) -> None:
        ...
        to populate the
        :attr:`~virtual_ecosystem.models.palms.palms_model.PalmsModel.pmodel`
        attribute.
    ...
    def allocate_gpp(self) -> None:
        ...
        This method uses palm growth form equations analogous to the T Model to
        estimate the allocation of plant gross primary productivity to respiration,
        growth, maintenance and turnover costs. The method then simulates growth by
        increasing stem height and calculates leaf and root turnover values.
        """
        ...
        for cell_id in self.communities.keys():
            ...
            #
            #       We could kill stems where the new H <=0 but adds loads of code and
            #       for the moment we just want to avoid passing negative sizes.
            #       If the np.where is removed and this is set directly, then
            #       StemAllometry will detect H <= 0 and behave unpredictably.
            #
            #       The problem with this hack is that it currently only
            #       targets stem height, not the rest of the allocation so these
            #       turnover etc may still be based on shrinking values.

            # Palm dbh_value is a fixed cohort input and is not updated by growth;
            # only stem_height_value grows.
            new_stem_height = (
                cohorts["stem_height_value"].to_numpy()
                + growth_increments.delta_height.squeeze()
            )
            cohorts["stem_height_value"] = np.where(
                new_stem_height <= 0,
                cohorts["stem_height_value"].to_numpy(),
                new_stem_height,
            )
    ...
    def update_allometry(self) -> None:
        """Update the palm growth form allometry of cohorts.

        This method is used to update the theoretical expectation of the stem
        allometry given any changes to the stem height of cohorts from growth. It also
        handles resetting LAI and foliage turnover rates - which may have been altered
        in the apply_herbivory method - to the default values for the PFT for each
        cohort.
        """
        ...
        for community in self.communities.values():
            ...
            # Update community allometry with new stem height values
    ...
    def apply_recruitment(self) -> None:
        ...
        This function currently only removes propagules from the propagule pool,
        following a single recruitment rate across all plant functional types.

        .. todo::

            This palm model does not yet capture juvenile palm growth: it is intended
            to represent palms from an early mature growth stage onward, matching
            plantation establishment. Recruited propagules are therefore not currently
            converted into new cohorts and are simply lost from the propagule pool.

            When juvenile growth is added, new recruit cohorts will need both an
            initial ``dbh_value`` and an initial ``stem_height_value`` set, since both
            are required to run the palm growth form equations for a given cohort -
            see :class:`~virtual_ecosystem.models.palms.communities.PalmCommunity`.
        """
        ...
        # Remove recruitment from propagule pool. Palms do not yet have a juvenile
        # growth stage (see docstring above), so these recruited propagules are lost
        # rather than being converted into new cohorts.
