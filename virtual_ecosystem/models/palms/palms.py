...
in height rather than diameter: ``dbh_value`` is a fixed cohort input that is not
expected to change with growth once a palm is mature, while ``stem_height_value`` is
the size measure that growth increments should update (see
:class:`~virtual_ecosystem.models.palms.communities.PalmCommunity`). The scaling
functions and the ``StemAllometry``, ``StemAllocation`` and ``GrowthIncrements``
classes below are expected to be edited accordingly.

::Note:
    The equations in this module are derived for mature palms. Juvenile growth forms are
    not yet adequately modelled. As such, recruitment is currently disabled. The model
    currently represents the growth of mature palms in a monoculture plantation setting.
...
def calculate_crown_areas(
    palm_a: NDArray[np.floating],
    palm_b: NDArray[np.floating],
    ...
    .. math::
        ...
        A_{c} =\frac{\pi}{4 b^2} \ln^2(\frac{H}{a})
    ...
    return (np.pi / (4 * palm_b**2)) * np.log(stem_height / palm_a) ** 2
...
def calculate_crown_fractions(
    ...
    return 1 / (stem_height * palm_b) * np.log(stem_height / palm_a)
...
def calculate_sapwood_masses(
    ...
    stem_height: NDArray[np.floating],
    dbh: NDArray[np.floating],
    palm_stem_resp_fraction: NDArray[np.floating],
) -> NDArray[np.floating]:
    r"""Calculate the mass of respiring tissue in palm stems.
    ...
    return palm_stem_resp_fraction * (np.pi / 8) * rho_s * (dbh**2) * stem_height
...
def calculate_sapwood_respiration(
    ...
    r"""Calculate living stem tissue respiration.
...
def calculate_foliage_turnover(
    ...
    For palms, this represents the carbon cost of dropped fronds, since palms
    have no branch turnover (see :func:`calculate_branch_turnover`).
...
def calculate_growth_increments(
    ...
    This function calculates growth increments for palm stems using analogous equations
    to those of the T Model :cite:`Li:2014bc`. Mature palms retain a fixed diameter at
    breast height while growing vertically throughout their lives. Estimated biomass 
    production (:math:`B`) can be partitioned into turnover costs (:math:`T`) and carbon
    available for allocation to biomass increments in:
    
    * the stem height (:math:`\Delta H`),
    ...
    * the fine root mass (:math:`\Delta W_r`).
    ...
    The stem height increment can be calculated using the available productivity for
    growth and the rates of change in stem mass (:math:`\textrm{d}W_s / \textrm{d}H`)
    and in the combined foliage and fine root masses (:math:`\textrm{d}W_fr /
    \textrm{d}H`) per unit of stem height:  

    .. math::

        \Delta H = \frac{B - T}{ \textrm{d}W_s / \textrm{d}H  +
             \textrm{d}W_fr / \textrm{d}H}

    The rates of change in stem and foliar mass with respect to stem height can be
    calculated as:
    ...
    .. math::
      ...
      \[
        \begin{align*}
            \textrm{d}W_s / \textrm{d}H &= \frac{\pi}{8} \rho_s D^2 \\

            \textrm{d}W_fr / \textrm{d}H &= \frac{\pi L}{2 b^2 H}
                \ln\left(\frac{H}{a}\right) \left(\frac{1}{\sigma} + \zeta \right)
        \end{align*}
      \]
    ...
    * the ratio of fine root mass to leaf area (:math:`\zeta`).
    ...
    = 0` then :math:`\Delta H` is undefined, otherwise :math:`\Delta H = \pm \inf`
    depending on whether then turnover costs exceed the available NPP. Under these
    conditions, this function explicitly sets :math:`\Delta H = 0`: **stems with zero
    height cannot grow**.
    ...
    .. math::
      ...
      \[
        \begin{align*}
        \Delta W_s &=  \textrm{d}W_s / \textrm{d}H \, \Delta H\\
        \Delta W_fr &=  \textrm{d}W_fr / \textrm{d}H \, \Delta H
        \end{align*}
      \]
    ...
    as a function of the increment in stem height (:math:`\Delta H`):
    ...
    dWfrdH = (
        (np.pi * lai)
        / (2 * stem_height * palm_b**2)
        * np.log(stem_height / palm_a)
        * (1 / sla + zeta)
    )

    # Increment of stem height, ignoring potential zero divides resulting
...
class StemAllometry(ToDataFrameMixin):
    ...
    def __init__(
        ...
        """A flag recording whether at_dbh was passed."""
        self._ndims: int
        ...
        # Palm stem height is a cohort input, not derived from dbh (see tree
        # calculate_heights).
        self.stem_height = cohorts.stem_height_value.to_numpy()

        self.crown_area = calculate_crown_areas(
            palm_a=cohorts.palm_a.to_numpy(),
            palm_b=cohorts.palm_b.to_numpy(),
        ...
        self.sapwood_mass = calculate_sapwood_masses(
            ...
            stem_height=self.stem_height,
            dbh=self.dbh,
            palm_stem_resp_fraction=cohorts.palm_stem_resp_fraction.to_numpy(),
        )
    ...
    def __repr__(self) -> str:
        """Return a description of the allometry predictions."""
        if self._at_dbh_set:
            return (
...
class StemAllocation(ToDataFrameMixin):
    ...
    Args:
        cohorts: An instance of :class:`~pyrealm.demography.cohorts.Cohorts`.
        allometry: An instance of
        :class:`~virtual_ecosystem.models.palms.palms.StemAllometry`.
    ...
    def __init__(
        ...
        """Allocation to leaf turnover (g C). For palms this is also the carbon cost
        of dropped fronds."""
        self.fine_root_turnover: NDArray[np.floating]
        """Allocation to fine root turnover"""
        self.branch_turnover: NDArray[np.floating]
        """Allocation to branch turnover. Always zero for palms, which have no woody
        branch fall."""
    ...
    def __repr__(self) -> str:
        """Return a description of the allocation predictions."""
...
class GrowthIncrements(ToDataFrameMixin):
    ...
    * calculated allocation of whole crown GPP estimates for the cohorts into
    ...
    def __init__(
        ...
        self.delta_height: NDArray[np.floating]
        """Predicted increase in stem height from growth allocation (m)"""
    ...
    def __repr__(self) -> str:
        """Return a description of the growth increment predictions."""
        match (self.profile, self._ndims):
            case (True, 3):
                repr_ = (
