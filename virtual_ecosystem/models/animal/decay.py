...
class SoilPool:
    ...
    def __init__(
        ...
        if pool_name == "pom":
            self.mass_cnp = self._extract_pom_cnp_mass(
                data=data,
                biotic_topsoil_depth=biotic_topsoil_depth,
            )
    ...
    def _extract_pom_cnp_mass(self, data: Data, biotic_topsoil_depth: float):
        """Extract the CNP masses of the :term`POM` soil pool.

        Args:
            data: The Virtual Ecosystem data object
            biotic_topsoil_depth: The soil depth below which the soil is no longer
                considered to be biologically active topsoil [m]
        """
        ...
        # soil active depth
        carbon_mass = carbon_stock * self.cell_area * biotic_topsoil_depth
        nitrogen_mass = nitrogen_stock * self.cell_area * biotic_topsoil_depth
        phosphorus_mass = phosphorus_stock * self.cell_area * biotic_topsoil_depth
