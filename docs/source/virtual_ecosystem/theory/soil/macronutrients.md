...
that they are generally thought to be the two macronutrients that most limit ecosystem
...
Nitrogen fixation generally produces ammonia ($\ce{NH_{3}}$), whereas ammonium
($\ce{NH_{4}^{+}}$) is generally produced during organic matter decomposition.
Transformation of ammonia to ammonium (and vice versa) is a frequent occurrence in
soils, but the process would be tricky to parametrise and validate. So, for the sake of
simplicity, we just track ammonium, under the assumption that ammonia and ammonium exist
in constant proportion. As well as the processes already mentioned ammonium can also be
generated by leaching from litter. This pool can be taken up by both plants and
microbes, and can be removed from the soil by water flows.
