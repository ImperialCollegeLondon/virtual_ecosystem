...
```{note}
The variables required to setup or update a particular model are often produced by other
models, and in some cases multiple models require the same variable. So, when
determining which variables you must provide to setup a specific model, you **need** to
check that variables aren't either generated by another model or already provided as
part of the setup of another model. This can be checked using the table below, but
unfortunately it is quite a finicky process at present.
```
